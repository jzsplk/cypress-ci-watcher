...
const parseVariableOpts = (fnArgs: [any, any], args: _.List<unknown> | null | undefined) => {
	...
	if ((unknownArgs && unknownArgs.length) && (opts.spec || opts.tag)) {
		...
		_.forEach(multiArgFlags, (flag) => {
			...
			const endIndex = nextOptOffset !== -1 ? argIndex + nextOptOffset : args?.length ?? 0
...
program
	...
	.action(async (...fnArgs: [any, any]) => {
		...
		console.log('pared fnArgs', parseVariableOpts(fnArgs, args));
